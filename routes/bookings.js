...
// --- BOOK AN ITEM FOR AN EVENT ---
// Allow Admin, Management, and Event Team to create bookings
router.post('/', protect, authorize('admin', 'management', 'event_team'), async (req, res) => { // CHANGED adminOnly to authorize
  const { item_id, event_id } = req.body;
  const booked_by_user_id = req.user.id; // User making the booking
  try {
    // Check for event existence and dates
